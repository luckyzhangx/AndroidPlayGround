...
fun RecyclerView.slowSmoothScrollToPositionTop(pos: Int) {
    if (pos == RecyclerView.NO_POSITION) return
    val lm = layoutManager
    if (lm is LinearLayoutManager) {
        val vPos = lm.findFirstVisibleItemPosition()
...
//open class CenterSmoothScroller : LinearSmoothScroller() {
...
fun RecyclerView.scrollToPositionWithOffset(pos: Int, offset: Int) {
    use(layoutManager) {
        when (this) {
            is LinearLayoutManager -> {
